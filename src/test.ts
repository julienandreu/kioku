...
void describe('memoize', () => {
	...
	/**
     * Test: Memoization with object having custom Symbol.toStringTag
     *
     * Covers the edge case where an argument has a custom Symbol.toStringTag property.
     */
	void test('memoization with object having custom Symbol.toStringTag', async _ => {
		let index = 0;
		const customTag = {[Symbol.toStringTag]: 'CustomTag'};
		const memoized = memoize((_?: unknown) => index++);
		strictEqual(memoized(customTag), 0);
		strictEqual(memoized(customTag), 0);
		strictEqual(index, 1);
	});

	/**
     * Test: Memoization with object without Symbol.toStringTag
     *
     * Covers the case where an object does not have a Symbol.toStringTag property.
     */
	void test('memoization with object without Symbol.toStringTag', async _ => {
		let index = 0;
		const plainObject = {foo: 'bar'};
		const memoized = memoize((_?: unknown) => index++);
		strictEqual(memoized(plainObject), 0);
		strictEqual(memoized(plainObject), 0);
		strictEqual(index, 1);
	});

	/**
     * Test: Memoization with array argument (object without Symbol.toStringTag)
     */
	void test('memoization with array argument (object without Symbol.toStringTag)', async _ => {
		let index = 0;
		const array = [1, 2, 3];
		const memoized = memoize((_?: unknown) => index++);
		strictEqual(memoized(array), 0);
		strictEqual(memoized(array), 0);
		strictEqual(index, 1);
	});

	/**
     * Test: Memoization with function argument (object without Symbol.toStringTag)
     */
	void test('memoization with function argument (object without Symbol.toStringTag)', async _ => {
		let index = 0;
		function fn() {
			return 'test';
		}
		...
		const memoized = memoize((_?: unknown) => index++);
		strictEqual(memoized(fn), 0);
		strictEqual(memoized(fn), 0);
		strictEqual(index, 1);
	});

	/**
     * Test: Memoization with object whose prototype has Symbol.toStringTag
     */
	void test('memoization with object whose prototype has Symbol.toStringTag', async _ => {
		let index = 0;
		const proto = {[Symbol.toStringTag]: 'ProtoTag'};
		const object = Object.create(proto);
		const memoized = memoize((_?: unknown) => index++);
		strictEqual(memoized(object), 0);
		strictEqual(memoized(object), 0);
		strictEqual(index, 1);
	});

	/**
     * Test: Memoization with object with null prototype (no Symbol.toStringTag)
     */
	void test('memoization with object with null prototype (no Symbol.toStringTag)', async _ => {
		let index = 0;
		const nullProtoObject = Object.create(null);
		const memoized = memoize((_?: unknown) => index++);
		strictEqual(memoized(nullProtoObject), 0);
		strictEqual(memoized(nullProtoObject), 0);
		strictEqual(index, 1);
	});
});
